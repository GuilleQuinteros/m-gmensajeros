...
export default function QRPage() {
    ...
    const iniciar = async () => {
      try {
        ...
        const tick = async () => {
          ...
          try {
            const result = await (reader as any).decodeFromVideoElement(videoRef.current);
            if (result && activo) {
              activo = false;
              detener();
          ...
          if (activo) rafRef.current = requestAnimationFrame(tick);
        };
    ...
    try {
      ...
      if (texto.startsWith("http")) {
        fetch(texto)
          ...
          .catch(() => setError("No se pudo obtener datos desde la URL del QR."));
      } else {
        setError(`Formato no reconocido: "${texto.substring(0, 60)}..."`);
      }
    }
  }
  ...
  function simularQR() {
    const ejemplo = [
      { nombre:"Juan", apellido:"Perez", dni:"28441220", telefono:"1145238812", direccion:"Av. Rivadavia 1234", localidad:"CABA", zonaId:"caba-24", zonaNombre:"CABA 24hs" },
      { nombre:"Maria", apellido:"Garcia", dni:"33812009", telefono:"1144569921", direccion:"Corrientes 890", localidad:"Quilmes", zonaId:"provincia-24", zonaNombre:"Provincia 24hs" },
    ];
  ...
  async function guardarTodos() {
    ...
    if (sinZona.length > 0 && !zonaId) {
      setError(`${sinZona.length} envio(s) sin zona. Selecciona una zona para todos.`);
      return;
    }
    setGuardando(true);
    setError("");
    ...
    <div>
      ...
      {envios.length === 0 && !scanning && (
        <div className="bg-white rounded-xl border border-gray-100 p-5 text-center mb-4">
          ...
          <p className="text-xs text-gray-400 mb-4">
            Apunta la camara al codigo QR con los pedidos del dia.
          </p>
      ...
      {scanning && (
        <div className="bg-white rounded-xl border border-gray-100 p-4 mb-4">
          <video
            ref={videoRef}
            style={{ width:"100%", borderRadius:8, minHeight:280, background:"#000" }}
            muted
            playsInline
          />
      ...
      {msg && <div className="bg-green-50 text-green-700 text-sm px-4 py-2.5 rounded-lg mb-4">{msg}</div>}
      ...
      {envios.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => { setEnvios([]); setMsg(""); }}
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-4 space-y-3">
            <div>
              <label className="block text-xs text-gray-500 mb-1">
                Zona para envios sin zona (opcional si el QR ya la trae)
              </label>
